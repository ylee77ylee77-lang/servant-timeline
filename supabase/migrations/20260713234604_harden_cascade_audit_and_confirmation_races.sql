-- Keep audit logging valid when deleting a service cascades into audited child
-- tables. The live FK is retained only while the parent service still exists;
-- the original service id remains available as an immutable metadata snapshot.
